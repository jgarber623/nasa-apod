...
process
  ...
  .forEach(async (date) => {
    ...
    const $anchor = $('center > h1 + p + p > br + a');
    const $image = $anchor.find('> img');

    const front_matter = {
      title: $('title').html().trim().split(' – ')[1],
      canonical_url,
      hd_image_url: $anchor.attr('href'),
      image_url: $image.attr('src'),
      image_alt: $image.attr('alt').replace(/\n|\s+/g, ' ').trim(),
      credit
    };

    const data = Buffer.from(`---json\n${JSON.stringify(front_matter, null, 2)}\n---\n\n${content}\n`);
